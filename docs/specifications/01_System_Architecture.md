...
> **SECTION 15 --- DEVELOPMENT ROADMAP**
  ...
  ----------- ---------------------- -------------- ----------------------------------------------------------------------------------------------------------------------------------------------------
  **Phase**   **Name**               **Duration**   **Objectives**

  Phase 1     System Foundation      4 weeks        FastAPI skeleton, PostgreSQL schema, Redis, Celery, Docker Compose stack, JWT auth, CI/CD pipeline, Alembic migrations, test framework

  Phase 2     Core Services          4 weeks        Genome Validation Service, file upload API, MinIO integration, job management API, WebSocket progress, Nextflow DSL2 skeleton, algorithm libraries

  Phase 3     AMR Detection          6 weeks        CARD RGI, AMRFinderPlus, ResFinder, Abricate integration; Alignment Service; Mutation Detection; result merging and deduplication; DB persistence

  Phase 4     Phenotype Prediction   4 weeks        Mechanism Classification Service; Phenotype Prediction rule engine; EUCAST/CLSI breakpoint integration; Virulence Profiling; Confidence Scoring

  Phase 5     Reporting              3 weeks        JSON/TSV/PDF report generation; Module 2 Export Service; presigned URL delivery; report storage; summary dashboard data feeds

  Phase 6     Module 2 Integration   3 weeks        module2_input.csv schema finalisation; integration testing with Module 2; schema versioning; batch export endpoint; concordance data validation
  ----------- ---------------------- -------------- ----------------------------------------------------------------------------------------------------------------------------------------------------
...
-   Objectives: Deployable infrastructure skeleton with auth, storage, and job queue operational.

-   Deliverables: Docker Compose stack, FastAPI app with /health and /auth endpoints, PostgreSQL schema v1 with Alembic, Redis + Celery operational, GitHub Actions CI with lint/test/build.

-   Acceptance Criteria: All infrastructure containers start and pass health checks; JWT auth flow end-to-end; test coverage ≥ 80% for auth layer.

-   Estimated Effort: 2 backend engineers × 4 weeks.

-   Risk: PostgreSQL schema design complexity; mitigated by domain-driven design workshop prior to sprint.

**Phase 2 --- Core Services**

-   Objectives: File upload pipeline operational; Nextflow workflow skeleton executing; algorithm libraries available.

-   Deliverables: Genome Validation Service with FASTA parsing and assembly metrics; MinIO file storage; Nextflow DSL2 module template; alignment algorithm library.

-   Acceptance Criteria: Upload → validation → job COMPLETED flow passes end-to-end test; S3 file integrity verified; Nextflow process executes in Docker.

-   Estimated Effort: 2 backend engineers + 1 bioinformatics engineer × 4 weeks.

-   Risk: Nextflow Docker executor configuration in compose environment; mitigated by early PoC.

**Phase 3 --- AMR Detection**

-   Objectives: Full AMR gene detection pipeline operational with all four databases.

-   Deliverables: CARD RGI, AMRFinderPlus, ResFinder, Abricate processes; Alignment Service; Mutation Detection Service; result merger; database persistence.

-   Acceptance Criteria: Known AMR-positive reference genomes produce expected gene calls at ≥ 95% recall; false positive rate \< 2% on AMR-negative controls.

-   Estimated Effort: 2 bioinformatics engineers × 6 weeks.

-   Risk: CARD RGI version dependency conflicts; mitigated by isolated Docker containers per tool.

**Phase 4 --- Phenotype Prediction**

-   Objectives: Mechanism classification and clinical phenotype prediction operational.

-   Deliverables: Mechanism Classification rule engine; Phenotype Prediction with EUCAST/CLSI breakpoints; Virulence Profiling; Confidence Scoring.

-   Acceptance Criteria: Phenotype predictions benchmarked against curated concordance dataset (≥ 85% concordance for HIGH confidence predictions).

-   Estimated Effort: 1 bioinformatics engineer + 1 backend engineer × 4 weeks.

**Phase 5 --- Reporting**

-   Objectives: Complete output artefact generation pipeline.

-   Deliverables: JSON/TSV/PDF report generators; Module 2 Export Service; file delivery API; report schema documentation.

-   Acceptance Criteria: PDF reports pass clinical review; TSV outputs importable into R/Excel; Module 2 CSV passes schema validation.

-   Estimated Effort: 1 backend engineer + 1 UI engineer × 3 weeks.

**Phase 6 --- Module 2 Integration**

-   Objectives: Verified, contract-stable integration between Module 1 and Module 2.

-   Deliverables: module2_input.csv schema v1.0 locked; integration test suite; batch export endpoint; schema version registry.

-   Acceptance Criteria: Module 2 engine successfully ingests Module 1 exports for 100 benchmark isolates; zero schema-breaking changes without version increment.

-   Estimated Effort: 1 backend engineer + 1 bioinformatics engineer × 3 weeks.

---

## SECTION 16 — MVP IMPLEMENTATION STRATEGY

> **This section describes the phased build order for a solo developer starting from scratch.**
> The full architecture in Sections 1–15 remains the target. This section defines what to build first.

### Phase 1 — MVP (Weeks 1–2): Real Science Working

Build a thin wrapper around existing tools. Get a FASTA in, get AMR genes out.

| Component | Tool | Status |
|-----------|------|--------|
| File upload | FastAPI + local disk | ✅ Build now |
| AMR detection | CARD RGI (Docker) | ✅ Build now |
| AMR detection | AMRFinderPlus (Docker) | ✅ Build now |
| Result aggregation | Python dict merge | ✅ Build now |
| JSON/TSV reports | Python json + csv | ✅ Build now |
| Database | SQLite via SQLAlchemy | ✅ Build now |
| Web UI | Plain HTML + fetch() | ✅ Build now |

**Stack:** FastAPI · SQLite · Local disk · Docker subprocess calls · No auth

**Reference:** See `2_Week_Beginner_Roadmap.md` for day-by-day instructions.

---

### Phase 2 — Add Depth (Weeks 3–6)

| Addition | Reference Document |
|----------|--------------------|
| Mutation detection | `07_Mutation_Mechanism_Engine.md` |
| ResFinder + Abricate integration | `06_AMR_Detection_Engine.md` |
| PostgreSQL (replace SQLite) | `02_Database_Design.md` |
| Genome validation engine | `04_Genome_Validation_Engine.md` |

---

### Phase 3 — Prediction Layer (Weeks 7–10)

| Addition | Reference Document |
|----------|--------------------|
| Phenotype prediction (S/I/R) | `08_Phenotype_Prediction_Engine.md` |
| EUCAST/CLSI breakpoints | `08_Phenotype_Prediction_Engine.md` |
| JWT authentication + RBAC | `03_API_Specification.md` |
| Celery + Redis (replace background tasks) | `10_Workflow_Deployment.md` |

---

### Phase 4 — Full Science (Weeks 11–14)

| Addition | Reference Document |
|----------|--------------------|
| Virulence profiling (VFDB) | `09_Virulence_Confidence_Engine.md` |
| Confidence scoring framework | `09_Virulence_Confidence_Engine.md` |
| Algorithm library (SW, NW, MinHash) | `05_Algorithm_Engine.md` |
| Module 2 export | `08_Phenotype_Prediction_Engine.md` |

---

### Phase 5 — Production Platform (Weeks 15+)

| Addition | Reference Document |
|----------|--------------------|
| Nextflow DSL2 orchestration | `10_Workflow_Deployment.md` |
| MinIO object storage | `10_Workflow_Deployment.md` |
| Docker Compose production stack | `10_Workflow_Deployment.md` |
| CI/CD pipeline | `10_Workflow_Deployment.md` |
| Kubernetes readiness | `10_Workflow_Deployment.md` |
