...
class ResistanceMutation(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    """A detected resistance mutation (SNP/Indel) for a sample."""
    ...
    mutation: Mapped[str] = mapped_column(String(100), nullable=False)
    mechanism: Mapped[str | None] = mapped_column(String(200), nullable=True)
    effect: Mapped[str | None] = mapped_column(String(200), nullable=True)
    ...
    __table_args__ = (
        Index("idx_resistance_mutations_sample", "sample_id"),
        Index("idx_resistance_mutations_job", "job_id"),
        Index("idx_resistance_mutations_gene", "gene_name"),
    )


__all__ = ["ResistanceMutation"]
